...
<p>Senior SEOs interview on exactly this distinction, and the analyzer enforces it end to end (Bot DB v2026.09.01, 64 signatures):</p>
<ul><li><strong>Training</strong> (GPTBot, ClaudeBot, CCBot, Bytespider, cohere-ai, AI2Bot, Google-Extended robots token): throttle/block freely at 60 req/min (20 aggressive). Zero live citation loss.</li><li><strong>Search-index</strong> (OAI-SearchBot, PerplexityBot, Claude-SearchBot, DuckAssistBot): ALLOW at 120 req/min (60 aggressive), 429 + Retry-After, never hard block — citation share drops in 1–2 weeks if blocked.</li><li><strong>User-triggered</strong> (ChatGPT-User, Perplexity-User, Claude-User, MistralAI-User, Google-Agent): do NOT throttle (300/min abuse ceiling only). A 429 here means a missing live answer; robots.txt may not even apply.</li></ul>
<h2>How the 10 modules work</h2>
<ul><li><strong>1 · Bot Classification:</strong> bot-signature matching takes precedence over browser fingerprints (documented bot-first order) across 64 signatures + 18 browser patterns.</li><li><strong>2 · Verification (4 layers):</strong> vendor IP JSON (dated 2026-09-01) first; short-prefix matches are labeled low-confidence heuristics, never verdicts. Anthropic publishes no IP list — robots.txt only. Signals, not proof.</li><li><strong>3 · Crawl Budget:</strong> per-crawler efficiency, parameterized-URL ratios, 13 trap patterns incl. gclid/fbclid, currency/locale and /filter/ segments.</li><li><strong>4 · Cost:</strong> measured bytes × your CDN preset (CloudFront/Cloudflare/Fastly/GCS). Blockable = training + suspicious only.</li><li><strong>5 · AI Matrix:</strong> red = training, cyan = search-index, blue = user-triggered, each with its 2026 rate policy.</li><li><strong>6 · Edge Rules + robots.txt:</strong> Cloudflare/Fastly/AWS rules with copy buttons, training-vs-search robots.txt generator (Google-Extended is a token, not a UA), Cloudflare AI Crawl Control mapping.</li><li><strong>7 · Performance:</strong> TTFB by category, status/method distributions.</li><li><strong>8 · Traffic Patterns:</strong> hourly spikes (mean + 2σ), day-of-week, top IPs/URLs/referrers.</li><li><strong>9 · Security:</strong> traversal, credential (.git/HEAD, .aws/credentials), actuator probes, velocity anomalies.</li><li><strong>10 · CFO/FinOps:</strong> measured totals, blockable waste, projections, CSV + print-to-PDF 1-pager export.</li></ul>
...
<ul><li>Classification is User-Agent matching — spoofable; use Verification signals + server-side rDNS before blocking.</li><li>Files above ~300k lines analyze a labeled systematic sample; 500MB+ exact totals belong in the CLI.</li><li>Cost needs your real CDN pricing to be more than directional — set a preset.</li></ul>`;
}

function renderHowto(){
  document.getElementById('howto-content').innerHTML=`
<h1>How To Use</h1>
<div class="step"><div class="step-n">1</div><div class="step-body"><h3>Prepare your log file</h3><p>Five formats, auto-detected per line — you can even mix JSON and Combined lines in one file. Uploads stream in 8MB slices, so 1GB files work; above ~300k lines a <strong>labeled systematic sample</strong> is analyzed. Each entry should carry timestamp, user_agent, IP, request URI and status (bytes/request_time/TLS/cache optional but recommended).</p></div></div>
...
<h2>FAQ</h2>
<div class="code"><h5>1GB upload fails?</h5><code>Use a modern desktop browser (not a phone), accept the &gt;500MB confirm, and stay on the tab. You will see Reading % then analysis %. Above ~300k lines a labeled 1-in-N sample is analyzed; costs in that case describe the sample and scale ~Nx.</code></div>
...
<div class="code"><h5>Download buttons?</h5><code>Sample downloads (1MB–1GB) stream in 20k-row chunks with progress. Analysis exports: Download CSV (bot costs) and CFO 1-pager (print-to-PDF) in the info bar after analysis.</code></div>`;
}
...
function processRecords(records,file,meta){
      ...
      try{
        if(records.length>20000&&typeof Worker!=='undefined'){
          const w=new Worker('js/worker.js?v=1.1.1');
