...
> [!NOTE]
> `main` branch. v2.2.0 · Bot DB v2026.09.17 (127 signatures) · IP JSON 2026-09-17 (14 sources incl. BotBase taxonomy, full CIDRs + IPv6). CFO export is a **real PDF download** built on vendored jsPDF 2.5.1 + AutoTable 3.8.2 (`js/vendor/`, MIT — no CDN, no runtime network). See [CHANGELOG.md](CHANGELOG.md) · [METHOD.md](METHOD.md).
...
Measured totals, blockable waste ($0.25 on the sample), projections, one-click CSV + real PDF download (`CFO PDF ↓` → `cfo-bot-traffic-cost-<start>-to-<end>.pdf`, vendored jsPDF + AutoTable in `js/vendor/`, no print dialog).
...
js/cfo-pdf.js               # CFO PDF on vendored jsPDF + AutoTable (no CDN/network at runtime): buildCFOData + generateCFOPDFBytes + downloadCFOPDF
js/vendor/                  # pinned jsPDF 2.5.1 + AutoTable 3.8.2 UMD (MIT, byte-identical to npm dist; see js/vendor/README.md)
